...
#
# This validates:
# - REST endpoints accept protojson(UIRequest) bodies (no legacy wrapper JSON)
# - protojson enum strings (type/status) remain stable (confirm/pending/etc)
...
submit_response() {
  local id="$1"
  local type="$2"
  local output_oneof_field="$3"
  local output_json="$4"
  post_json "${API_BASE_URL}/api/requests/${id}/response" "{\"type\":\"${type}\",\"sessionId\":\"global\",\"${output_oneof_field}\":${output_json}}"
}
...
REQ="$(create_request "select" "selectInput" '{"title":"INSPECTOR select","options":["a","b","c"],"multi":false,"searchable":true}')"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "select" "selectOutput" "{\"selectedSingle\":\"b\",\"comment\":\"CURL_OK\"}" >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '.selectOutput.selectedSingle == "b"' >/dev/null

say "select: selectedMulti"
REQ="$(create_request "select" "selectInput" '{"title":"INSPECTOR select multi","options":["a","b","c"],"multi":true,"searchable":true}')"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "select" "selectOutput" "{\"selectedMulti\":{\"values\":[\"a\",\"c\"]},\"comment\":\"CURL_OK\"}" >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '.selectOutput.selectedMulti.values | length == 2' >/dev/null

say "form: struct schema + struct output"
REQ="$(create_request "form" "formInput" '{"title":"INSPECTOR form","schema":{"type":"object","properties":{"host":{"type":"string"}},"required":["host"]}}')"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "form" "formOutput" '{"data":{"host":"example"},"comment":"CURL_OK"}' >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '.formOutput.data.host == "example"' >/dev/null

say "table: data[] + selectedSingle"
REQ="$(create_request "table" "tableInput" '{"title":"INSPECTOR table","data":[{"id":"srv-1","name":"alpha"},{"id":"srv-2","name":"beta"}],"columns":["id","name"],"multiSelect":false,"searchable":true}')"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "table" "tableOutput" '{"selectedSingle":{"id":"srv-2","name":"beta"},"comment":"CURL_OK"}' >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '.tableOutput.selectedSingle.id == "srv-2"' >/dev/null

say "upload: files[]"
REQ="$(create_request "upload" "uploadInput" '{"title":"INSPECTOR upload","accept":[".txt"],"multiple":true,"maxSize":1234}')"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "upload" "uploadOutput" '{"files":[{"name":"a.txt","size":1,"path":"/tmp/a.txt","mimeType":"text/plain"}],"comment":"CURL_OK"}' >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '.uploadOutput.files[0].name == "a.txt"' >/dev/null

say "image: variant A (selectedNumber)"
REQ="$(create_request "image" "imageInput" "{\"title\":\"INSPECTOR image\",\"message\":\"pick\",\"images\":[{\"src\":\"${API_BASE_URL}${IMG_URL}\",\"label\":\"A\"},{\"src\":\"${API_BASE_URL}${IMG_URL}\",\"label\":\"B\"}],\"mode\":\"select\",\"options\":[],\"multi\":false}")"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "image" "imageOutput" "{\"selectedNumber\":0,\"timestamp\":\"$(date -Is)\",\"comment\":\"CURL_OK\"}" >/dev/null
DONE="$(wait_request "$ID" 10)"
echo "$DONE" | jq -e '(.imageOutput.selectedNumber|tonumber) == 0' >/dev/null

say "image: confirm mode (selectedBool)"
REQ="$(create_request "image" "imageInput" "{\"title\":\"INSPECTOR image confirm\",\"message\":\"similar?\",\"images\":[{\"src\":\"${API_BASE_URL}${IMG_URL}\"}],\"mode\":\"confirm\",\"options\":[],\"multi\":false}")"
ID="$(echo "$REQ" | jq -r '.id')"
submit_response "$ID" "image" "imageOutput" "{\"selectedBool\":true,\"timestamp\":\"$(date -Is)\",\"comment\":\"CURL_OK\"}" >/dev/null
DONE="$(wait_request "$ID" 10)"
